...
public class ItemListView extends Div implements BeforeEnterObserver {
    ...
    private void deleteItem(Long id) {
        try {
            // Call the backend service to delete the item
            inventoryListService.delete(id);  // Ensure this matches the method name in your service class
            refreshGrid();  // Refresh the grid to update the UI and remove the deleted item
            Notification.show("Item deleted successfully", 3000, Notification.Position.BOTTOM_CENTER);  // Show success notification
        } catch (Exception e) {
            Notification.show("Failed to delete the item: " + e.getMessage(), 5000, Notification.Position.BOTTOM_CENTER);  // Show error notification if deletion fails
        }
    }
    ...
    public ItemListView(InventoryListService inventoryListService) {
        ...
        grid.addColumn(itemImageRenderer).setHeader("Item Image").setWidth("128px").setFlexGrow(0);
